...
of security to local accounts. Users can enroll authenticator apps such as
...
When enabled, all local users must complete MFA verification after login before
...
2. If MFA is required (`mfa_required: true`) **or** the user has enrolled MFA
...
- **`mfa_required: true`**: All users must complete MFA verification. Users
  without enrolled devices will be prompted to set up a device before they can
