...
Complete AWS deployment documentation for Safebucket, covering both basic
infrastructure and ECS deployment options.
...
Safebucket on AWS provides a complete cloud storage solution with two deployment
options:
...
- **ElastiCache Redis Cluster** - Caching and session storage with user
  authentication
...
- **Monitoring**: Enhanced monitoring with 60-second intervals, Performance
  Insights enabled
- **Security**: Private subnets only, VPC security groups
- **Parameters**: Optimized configuration with statement logging and
  pg_stat_statements
...
All services run on **AWS Fargate** with automated health checks and logging to
CloudWatch.
...
| `jwt_secret`       | Token secret (32+ chars)       | `your-jwt-secret-32-chars-minimum-here`                       |
...
3. **Access Services:**
   ...
   - **Loki:** Internal only (access via checking logs in S3 or ECS Exec if
     needed)
...
- Check DATABASE\_\* environment variables in task definition
